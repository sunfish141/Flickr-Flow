...
from wildfire_data.web.live_firms import LiveFirmsError, aggregate_current_firms
...
class WebAppTests(unittest.TestCase):
    def setUp(self):
        ...
        self.client = TestClient(create_app(allowed_hosts=['testserver'], model=model, terrain_provider=terrain, landscape=FuelPatch()))
    ...
    def test_seed_step_and_stateless_replay(self):
        ...
        self.assertGreater(advanced['new_ignition_count'], 0)
        self.assertLessEqual(advanced['new_ignition_count'], 8)
        survivor = next(c for c in advanced['state']['active_cells']
                        if c['cell_id'] == initial['state']['active_cells'][0]['cell_id'])
        self.assertLess(survivor['fuel_remaining'], 1.)
    ...
    def test_low_intensity_seed_burns_out_after_two_api_steps(self):
        ...
        seed = frame['state']['active_cells'][0]['cell_id']
        for step in (1, 2):
            ...
            frame = response.json()
            if step == 1:
                cell = next(c for c in frame['state']['active_cells'] if c['cell_id'] == seed)
                self.assertEqual((cell['fuel_remaining'], cell['remaining_active_steps']), (.5, 1))
            else:
                self.assertIn(seed, frame['state']['burned_cell_ids'])
                self.assertNotIn(seed, [c['cell_id'] for c in frame['state']['active_cells']])

    def test_fuel_survives_api_roundtrip_and_exhausted_fire_stays_out(self):
        frame = self.seed()
        original = frame['state']['active_cells'][0]
        original['fuel_remaining'] = 0.
        response = self.client.post('/api/step', json={'state': frame['state'], 'origin_at': frame['origin_at']})
        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.json()['extinct'])
        self.assertEqual(response.json()['new_ignition_count'], 0)

    def test_water_placement_is_rejected(self):
        model = IncidentTransitionModel(SpreadEstimator(), feature_columns=RECURSIVE_MODEL_FEATURE_COLUMNS)
        with TestClient(create_app(allowed_hosts=['testserver'], model=model, terrain_provider=terrain)) as client:
            response = client.post('/api/seed', json={'ignitions': [dict(latitude=47.7, longitude=-87.5, intensity=.7)]})
            self.assertEqual(response.status_code, 422)
            self.assertIn('mapped land', response.json()['detail'])

    def test_firms_water_cells_are_excluded_and_reported(self):
        def loader(_key, bounds, *, now):
            now = datetime(2026, 9, 5, 12, tzinfo=timezone.utc)
            rows = [{"latitude": lat, "longitude": lon, "acquired_at": '2026-09-05T06:00:00Z',
                     "bright_ti4": 290., "provenance": {"product": "VIIRS_SNPP_NRT"}}
                    for lat, lon in [(47.7, -87.5), (53.02, -117.31)]]
            return aggregate_current_firms(rows, bounds, now=now)
        model = IncidentTransitionModel(SpreadEstimator(), feature_columns=RECURSIVE_MODEL_FEATURE_COLUMNS)
        with TestClient(create_app(allowed_hosts=['testserver'], model=model, terrain_provider=terrain, firms_loader=loader)) as client:
            response = client.post('/api/firms')
            self.assertEqual(response.status_code, 200)
            result = response.json()
            self.assertEqual(result['active_count'], 1)
            self.assertEqual(result['metadata']['water_cells_excluded'], 1)
            self.assertEqual(result['state']['active_cells'][0]['bright_ti4_max'], 290.)
            self.assertGreater(result['state']['active_cells'][0]['intensity'], 0.)
