...
@dataclass
class SupportedFeatureEstimator:
    """Ignore columns with no training observations, retaining bundle ordering.

    An entirely missing training feature cannot learn a split. Selection is
    fitted from training rows alone and remains fixed for calibration/serving.
    Values are never filled with fabricated observations.
    """
    ...
    def predict_proba(self, values):
        ...
        if values.ndim != 2 or values.shape[1] != self.n_features_in_:
            raise ValueError("input columns differ from fitted support contract")
        return self.estimator.predict_proba(values[:, self.supported_indices])
...
class IncidentTransitionModel(RecursiveTransitionModel):
    ...
    def step(self, state, *, terrain_provider, origin_at=None):
        result = super().step(state, terrain_provider=terrain_provider, origin_at=origin_at)
