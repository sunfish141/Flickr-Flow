...
from wildfire_data.model.features.schema import DEFAULT_CANDIDATE_RADIUS_CELLS, DEFAULT_MODEL_FEATURE_COLUMNS
from wildfire_data.model.features.schema import feature_set_for_columns, validate_feature_contract, VEGETATION_COLUMNS
from wildfire_data.core.grid import GridCell, cell_from_id, cell_from_wgs84, cells_in_square_radius
...
class ActiveFireCell:
    ...
    fuel_remaining: float = field(default=1., kw_only=True)
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "fuel_remaining", _unit_interval(self.fuel_remaining, "fuel_remaining"))
...
class RecursiveTransitionModel:
    ...
    def __init__(
        ...
        *,
        ...
        try:
            self.feature_set = feature_set_for_columns(columns)
        except ValueError:
            raise RecursiveTransitionError(
                "model feature contract does not match the recursive frontier features"
            ) from None
        self.vegetation_columns = tuple(c for c in columns if c in VEGETATION_COLUMNS)
        self.vegetation_sampler = vegetation_sampler
        if self.vegetation_columns and vegetation_sampler is None:
            raise RecursiveTransitionError("vegetation model requires its pinned vegetation sampler")
        fitted = getattr(estimator, "model", estimator)
        if getattr(fitted, "n_features_in_", len(columns)) != len(columns):
            raise RecursiveTransitionError("estimator input count differs from feature contract")
        if (
    ...
    def from_model_bundle(
        ...
        **parameters: Any,
        ...
        try:
            ...
            raise RecursiveTransitionError(str(exc)) from exc
        ...
        if contract.get("vegetation"):
            from wildfire_data.model.features.vegetation_features import VegetationFeatureSampler
            reference = contract["vegetation"]
            parameters["vegetation_sampler"] = VegetationFeatureSampler(reference["manifest_path"],
    ...
    def step(
        ...
        *,
        ...
        candidate_features = self.candidate_feature_rows(
            state, terrain_provider=terrain_provider, origin_at=origin_at
        )
    ...
    def candidate_feature_rows(
        ...
        *,
        ...
        return tuple(
            (
                cell,
                self._complete_feature_row(cell.cell_id,
                    self._feature_row(cell, active_by_id=active_by_id, terrain_provider=terrain_provider),
                    origin_at=origin_at, step_index=state.step_index),
            )
            for cell in self.candidate_cells(state)
            if included is None or cell.cell_id in included
        )

    def _complete_feature_row(self, cell_id, features, *, origin_at, step_index):
        if self.vegetation_columns:
            from wildfire_data.model.features.vegetation_features import utc
            origin = utc(origin_at)
            sample = self.vegetation_sampler.sample_cell(cell_id, cutoff_at=origin,
                simulation_at=origin + timedelta(hours=12 * step_index))
            features = {**features, **{c: sample[c] for c in self.vegetation_columns}}
