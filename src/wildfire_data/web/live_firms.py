...
from wildfire_data.providers.fires.firms_normalization import normalize_firms_detection
from wildfire_data.providers.fires.firms_normalization import DEFAULT_BBOX
from wildfire_data.model.incident_transition import EvidenceCell
from wildfire_data.model.recursive_transition import RecursiveFireState
...
def fetch_current_firms(api_key, bounds=DEFAULT_BOUNDS, *, now=None, session=None):
    ...
    if not api_key:
        raise LiveFirmsError("Set NASA_FIRMS_API_KEY or MAP_KEY in the server environment or this repository's config/.env, then restart the server.")
