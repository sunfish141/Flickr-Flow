...
export function useScenario() {
  ...
  const [scenario, setScenario] = useState({ history: [], cursor: 0, ignitions: [], source: 'placed' });
  ...
  const activeKind = useRef(null);
  const generation = useRef(0);
  const message = useCallback((text, error = false) => setStatus({ text, error }), []);
  const pause = useCallback(() => {
    if (activeRequest.current) message('Request canceled. The previous frame is retained; server preparation may still be finishing.');
    generation.current++;
  ...
  useEffect(() => {
    ...
    return () => { controller.abort(); generation.current++; activeRequest.current?.abort(); };
  }, [message]);
  useEffect(() => {
    const hide = () => {
      if (!document.hidden) return;
      // Pause playback, but let an explicitly requested seed/FIRMS load finish.
      // Aborting those loads left an empty map with a permanent loading message.
      if (activeKind.current && !activeKind.current.endsWith('step')) setPlaying(false);
      else pause();
    };
    document.addEventListener('visibilitychange', hide);
    return () => document.removeEventListener('visibilitychange', hide);
  }, [pause]);
  const request = useCallback(async (kind, body, onSuccess) => {
    ...
    setBusy(kind);
    try {
      const result = await api(`/api/${kind}`, body, controller.signal);
      if (token !== generation.current) return false;
      onSuccess(result);
      return true;
    } catch (error) {
      if (error.name !== 'AbortError' && token === generation.current) {
      ...
      if (token === generation.current) { activeRequest.current = null; activeKind.current = null; setBusy(null); }
    }
  }, [message]);
  const frame = scenario.history[scenario.cursor];
  const advance = useCallback(async () => {
    if (activeRequest.current || !frame || frame.finished) return;
    if (scenario.cursor < scenario.history.length - 1) {
      if (scenario.history[scenario.cursor + 1].finished) setPlaying(false);
      setScenario(value => ({ ...value, cursor: value.cursor + 1 })); return;
    }
    ...
    return request(frame.expanding ? 'landscape/step' : frame.local ? 'local/step' : 'step', { state: frame.state, origin_at: frame.origin_at,
      ...
      setScenario(value => {
        const history = [...value.history, result].slice(-HISTORY_LIMIT);
        return { ...value, history, cursor: history.length - 1 };
      });
  ...
  }, [frame, scenario.cursor, scenario.history.length, request, message]);
  useEffect(() => {
  ...
  const addIgnition = async (latitude, longitude, intensity) => {
    ...
    return request(localRegion === 'auto' ? 'landscape/seed' : localRegion ? 'local/seed' : 'seed', { ignitions, ...(localRegion && localRegion !== 'auto' ? { region: localRegion } : {}) }, result => {
      setScenario({ history: [result], cursor: 0, ignitions, source: 'placed' });
  ...
  const loadFirms = (bounds, date = null) => request((localRegion === 'auto' ? 'landscape/' : '') + (date ? 'firms/historical' : 'firms'), date ? { date, bounds } : bounds, result => {
    setScenario({ history: [result], cursor: 0, ignitions: [], source: 'firms' });
    if (result.expanding) {
  ...
  const reset = () => { pause(); setScenario({ history: [], cursor: 0, ignitions: [], source: 'placed' }); message('Scenario reset.'); };
  const seek = index => { pause(); setScenario(value => ({ ...value, cursor: Math.max(0, value.history.findIndex(frame => frame.state.step_index === index)) })); message(''); };
  return { config, scenario, frame, playing, busy, busySeconds, firmsCooldown, speed, setSpeed, status, message,
    ...
    play: () => { if (frame?.finished) return; message(''); setPlaying(true); },
  };
}
