...
export function useScenario() {
  ...
  useEffect(() => {
    ...
    api('/api/config', undefined, controller.signal).then(data => {
      setConfig(data);
      if (!data.local_spread?.available && !data.model_ready) message('No simulation data is available. The overview map remains available.', true);
  ...
  useEffect(() => {
    const networkChanged = event => {
      if (event.origin === location.origin && event.source === window.parent && event.data?.type === 'wildfire:workspace' && !event.data.visible) { pause(); return; }
      if (event.origin !== location.origin || event.source !== window.parent || event.data?.type !== 'wildfire:network') return;
      const { online_enabled, firms_configured } = event.data.status;
      setConfig(previous => previous?.desktop ? { ...previous, firms_configured,
        desktop: { ...previous.desktop, online_enabled } } : previous);
      if (!online_enabled && requests.current.active?.kind.includes('firms') && !requests.current.active.kind.includes('historical')) pause();
    };
    window.addEventListener('message', networkChanged);
    return () => window.removeEventListener('message', networkChanged);
  }, [pause]);
  ...
  const addIgnition = async (latitude, longitude, intensity, endpoint = 'map/seed') => {
  ...
  const reset = () => { pause(); dispatch({ type: 'reset' }); setLocalRegion(''); message('Scenario reset.'); };
