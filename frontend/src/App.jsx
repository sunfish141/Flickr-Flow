  ...
  return <>
    ...
    <main className="workspace">
      <aside id="scenario-controls" tabIndex={-1} className="sidebar" aria-label="Scenario controls">
        ...
        <p className="research-note">Research simulation. {sim.expanding && sim.weatherML ? 'Experimental weather ML and polygon coupling.' : 'Fixed scenario conditions.'} Do not use for evacuation or emergency decisions; follow local authorities.</p>
        <div className="tabs" role="group" aria-label="Starting state source">
          ...
          {config?.local_spread?.available && <div className="local-scenario-controls">
            ...
            {sim.expanding && <div className="weather-controls">
              <label><input id="weather-ml" type="checkbox" checked={sim.weatherML} disabled={!!busy || playing || !config.local_spread.weather_ml?.available} onChange={e => { sim.setWeatherML(e.target.checked); setPlacing(false); setSelectedCell(null); }} /> Weather ML + polygon (experimental)</label>
              <p className="hint">{config.local_spread.weather_ml?.available ? 'Uses forecast weather for placed/current fires and historical analysis for historical scenarios. One weather location applies to the whole scenario. Beyond the forecast, the last conditions continue as an explicit assumption.' : 'A trained weather model and terrain provider are required on this server.'}</p>
            </div>}
            ...
            {sim.localRegion && <p className="hint">Experimental {config.local_spread.mesh_m} m fuel patches with road barriers. Uncalibrated travel rates with {sim.expanding && sim.weatherML ? 'weather-driven wind and ML cell admission' : 'constant scenario wind'}; urban mixtures and structures are unsupported. Unknown road widths use a {config.local_spread.policy.unknown_road_width_m} m assumption where surface type is mapped. {sim.expanding ? 'Roads are read from a local archive. Landscape tiles are built as the fire spreads; first preparation may take several minutes.' : 'Place within the selected region.'}</p>}
          </div>}
        ...
        <section id="place-panel" hidden={mode !== 'place'} aria-label="Place starting fires">
          {sim.expanding && sim.weatherML && <div className="historical-controls">
            <label htmlFor="weather-date">Historical weather date (optional)</label>
            <input id="weather-date" type="date" min="2026-05-11" max="2026-08-21" value={sim.weatherDate} disabled={!!busy || playing} onChange={e => { sim.setWeatherDate(e.target.value); setPlacing(false); setSelectedCell(null); }} />
            <p className="hint">Leave blank to use the current forecast. Choose a date to place a fire with historical weather, without requiring historical satellite detections.</p>
          </div>}
    ...
    <dialog id="help-dialog" ref={help} aria-labelledby="help-title"><button id="close-help" className="icon-button" aria-label="Close instructions" onClick={() => help.current.close()}>×</button>
      ...
      <h3>Research limitations</h3><p>One possible simulation, not an operational forecast. Cover-based fuel amounts and burning durations are uncalibrated scenario assumptions; dead wood, litter and fuel moisture are not measured. The 1 km model has no weather inputs. Standard polygon spread uses constant scenario wind; optional weather ML polygon mode uses captured weather and synthetic fire features, with an unvalidated coupling between 1 km probabilities and 30 m travel. Intensity is a scenario scale, not fire radiative power. Mapped water excludes whole 1 km cells; unresolved waterways may still be missed. Evaluation covers 12–96 hours; longer playback does not establish accuracy. Follow local emergency authorities.</p>
      <h3>Data and attribution</h3><p>Observations: <a href="https://www.earthdata.nasa.gov/data/tools/firms">NASA FIRMS</a>, VIIRS SNPP, NOAA-20 and NOAA-21 NRT. Context where available: NOAA ETOPO 2022, NALCMS land cover, NASA MOD44B and MOD13Q1. Water barriers: <a href="https://www.naturalearthdata.com/about/terms-of-use/">Natural Earth</a>. Weather ML: <a href="https://open-meteo.com/">Open-Meteo / ECMWF IFS</a>. Optional basemap: <a href="https://www.openstreetmap.org/copyright">© OpenStreetMap contributors, ODbL</a>.</p>
      <h3>Privacy</h3><p>Scenarios live in this tab’s memory and clear on reload. Coordinates and scenario states are sent to this app’s server for calculation. Live FIRMS requests send the selected bounds to NASA through the server; historical mode reads the server’s retained archive. Weather ML sends a rounded representative ignition location and dates to Open-Meteo through the server; its captured weather is cached on the server for replay. This app adds no analytics, cookies, or browser storage. Hosting providers may retain access logs. The OpenStreetMap basemap sends your IP, site origin, and tile coordinates to its provider; it loads automatically and can be disabled under Map layers.</p>
