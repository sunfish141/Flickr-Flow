...
export default function App() {
  ...
  return <>
    ...
    <dialog id="help-dialog" ref={help} aria-labelledby="help-title"><button id="close-help" className="icon-button" aria-label="Close instructions" onClick={() => help.current.close()}>×</button>
      ...
      <h3>How fire becomes burned</h3><p>The 1 km model consumes half of each cell's initial simulated fuel every 12 hours, normally burning out 24 hours after ignition. Polygon spread tracks each fuel patch's ignition time separately; a patch becomes burned after its configured burning duration{config?.local_spread?.policy && <> ({config.local_spread.policy.residence_minutes} simulated minutes on this server)</>}. A 1 km summary cell can contain both active and burned patches. Historical playback advances two 12-hour steps per day. Burned fuel does not reignite in the same scenario.</p>
      <h3>Research limitations</h3><p>One possible simulation, not an operational forecast. Burning durations are scenario assumptions, not measured fuel depletion. The 1 km model has no weather inputs; polygon spread uses constant scenario wind. Intensity is a scenario scale, not fire radiative power. Small waterways may be missed. Evaluation covers 12–96 hours; longer playback does not establish accuracy. Follow local emergency authorities.</p>
